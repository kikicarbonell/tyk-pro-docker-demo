...
ORG_DATA=$(curl --silent --header "admin-auth: 12345" --header "Content-Type:application/json" --data '{"owner_name": "TestOrg5 Ltd.","owner_slug": "testorg", "cname_enabled":true, "cname": "'$TYK_PORTAL_DOMAIN'"}' http://$DOCKER_IP/admin/organisations 2>&1)
ORG_ID=$(echo $ORG_DATA | $PYTHON_BIN -c 'import json,sys;obj=json.load(sys.stdin);print(obj["Meta"])')
echo "ORG ID: $ORG_ID"

echo "Adding new user"
USER_DATA=$(curl --silent --header "admin-auth: 12345" --header "Content-Type:application/json" --data '{"first_name": "John","last_name": "Smith","email_address": "'$TYK_DASHBOARD_USERNAME'","active": true,"org_id": "'$ORG_ID'"}' http://$DOCKER_IP/admin/users 2>&1)
USER_AUTH=$(echo $USER_DATA | $PYTHON_BIN -c 'import json,sys;obj=json.load(sys.stdin);print(obj["Message"])')
USER_LIST=$(curl --silent --header "authorization: $USER_AUTH" http://$DOCKER_IP/api/users 2>&1)
